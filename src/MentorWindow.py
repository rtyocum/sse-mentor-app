...
class MentorWindow(QMainWindow):
    ...
    def checkin_test(self, user: tuple, test_checkout: tuple) -> None:
        test_checkin = TestCheckinDialog(self, test_checkout, user)
        if test_checkin.exec() == QDialog.DialogCode.Accepted and test_checkin.success:
            if test_checkin.action == TestCheckinAction.RETURN:
                cursor = self.db_conn.execute("UPDATE testcheckout SET checkin_date = ? WHERE ID = ?", (datetime.datetime.now(), test_checkout[0]))
                self.db_conn.commit()
                cursor.close()
    ...
    def checkout_test(self, user: tuple) -> bool:
        ...
        test_checkout = TestCheckoutForm(self, user)
        if test_checkout.exec() == QDialog.DialogCode.Accepted and test_checkout.success:
            cursor = self.db_conn.execute("INSERT INTO testcheckout (user_id, course_code, test_name, checkout_date) VALUES (?, ?, ?, ?)", (user[0], test_checkout.course_code, test_checkout.exam, datetime.datetime.now()))
            self.db_conn.commit()
            cursor.close()
            return True
        return False

    def get_current_test(self, user_id: int) -> tuple:
        """Get the current test that the user has checked out."""
        cursor = self.db_conn.execute("SELECT ID, COURSE_CODE, TEST_NAME FROM testcheckout WHERE user_id = ? AND checkin_date IS NULL", (user_id,))
        test = cursor.fetchone()
        cursor.close()
        return test

    def get_user_by_uid(self, uid: str) -> tuple:
        """Get a user by the given UID."""
        hashed_uid = hashlib.sha512(uid.encode()).hexdigest()
        cursor = self.db_conn.execute("SELECT ID, EMAIL, FIRSTNAME, LASTNAME FROM users WHERE uid = ?", (hashed_uid,))
        user = cursor.fetchone()
        cursor.close()
        return user
    
    def register_user(self, uid: str) -> tuple:
        ...
        if user_registration.exec() == QDialog.DialogCode.Accepted and user_registration.success:
            hashed_uid = hashlib.sha512(user_registration.uid.encode()).hexdigest()
            self.db_conn.execute("INSERT INTO users (uid, firstname, lastname, email) VALUES (?, ?, ?, ?)", (hashed_uid, user_registration.first_name, user_registration.last_name, user_registration.email))
            self.db_conn.commit()
    ...
    def publish_record(self, test_checkout_id: int) -> None:
        ...
        sql = "SELECT tc.COURSE_CODE, tc.TEST_NAME, tc.CHECKOUT_DATE, tc.CHECKIN_DATE, u.FIRSTNAME, u.LASTNAME, u.EMAIL FROM testcheckout tc JOIN users u ON tc.USER_ID = u.ID WHERE tc.ID = ?"
        ...
        res = requests.post("https://oauth2.googleapis.com/token", json={"grant_type": "urn:ietf:params:oauth:grant-type:jwt-bearer", "assertion": token})
        res.raise_for_status()

        access_token = res.json()["access_token"]

        sheetId = '1MTez2TViiAPHDTIKbGvbD-bk6hP01qO--ioPwTQGjeQ'
        range = 'A1'
